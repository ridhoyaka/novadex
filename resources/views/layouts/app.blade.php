...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-arsa-900" x-data="{ sidebarOpen: false }">
        <div class="flex h-screen overflow-hidden">
            ...
            <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'"
                   class="fixed inset-y-0 left-0 z-50 w-64 bg-arsa-950 border-r border-arsa-800 flex flex-col transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-auto lg:z-auto">
                <!-- Logo -->
                ...
                <nav class="flex-1 overflow-y-auto py-6 px-3">
                    ...
                    <!-- Public Link -->
                    <a href="{{ route('home') }}" class="flex items-center gap-3 px-4 py-3 rounded-lg text-gray-300 hover:bg-arsa-800 hover:text-gold-400 transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="border-t border-arsa-800 p-4">
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 bg-gold-500 rounded-full flex items-center justify-center text-arsa-900 font-bold">
            ...
            <div class="flex-1 flex flex-col overflow-hidden">
                <!-- Top Bar -->
                <header class="h-16 bg-arsa-900 border-b border-arsa-800 flex items-center justify-between px-4 lg:px-6">
                    ...
                    <div class="flex-shrink-0 ml-3">
                        @if(auth()->user()->isSuperAdmin())
                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-gold-500/10 text-gold-400">Super Admin</span>
                        @elseif(auth()->user()->isAdmin())
                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-purple-500/10 text-purple-400">Admin</span>
                        @elseif(auth()->user()->isUmkm())
                        <span class="px-3 py-1 text-xs font-semibold rounded-full bg-blue-500/10 text-blue-400">UMKM</span>
                        @endif
                    </div>
                </header>
